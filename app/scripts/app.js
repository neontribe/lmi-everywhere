...
function getWageInfo(soc) {
	...
	$.ajax({
		...
		var sum = wagesByRegion.breakdown.reduce(function(a, b) {
			return a + b;
		});
...
(function($, undefined){
		...
		function updateRegion(region) {
			...
			// Populate region drop-down menu.
			render($('#search').find('#region'), 'region_select', { regions: regions, sel: app.region });

			// Update or initialise region drop-down menu.
		...
		/**
		 * Ensures a passed value is actually a string otherwise throws an error
		 */
		...
		/**
		 * Displays a message overlay for the specified time
		 */
		...
		/**
		 * Shows a jQuery mobile popup with the specified message.
		 */
						...
						$.ajax({
							...
							data: {
								...
								getWageInfo(app.soc).then(function(wdata){
									...
									/**
									 * Ensure a search term is set, especially for the iframe version which would
									 * not otherwise has a value at this point.
									 */
									if (!app.searchTerm) {
									...
									if (typeof Rickshaw !== 'undefined') {
										...
										yaxis = new Rickshaw.Graph.Axis.Y({
											graph: chart,
											orientation: 'right',
											//tickFormat: Rickshaw.Fixtures.Number.formatKMBT, // Short number format.
										...
										var plotData = [{ data: x, label: 'Data'}];
				...
				promise = $.Deferred(function(d) {
					fetchSOC(app.soc).then(function() {
						$.ajax({
							...
							/**
							 * Defines the output for a region click popup.
							 */
							function getTrendOutput(name) {
								...
								var a = parseInt(currentemp.replace(/,/g, ''), 10);
								var b = parseInt(lastemp.replace(/,/g, ''), 10);
							...
							d3.json('uk_euregions.json', function(error, uk) {
								var width		= $(window).width(),
										...
										regions = topojson.feature(uk, uk.objects['uk_regions']),
										dcolor	= d3.scale.linear().domain([-1,0]).range(['#ff0000', '#ff9999']),
										icolor	= d3.scale.linear().domain([1,0]).range(['#ffa500', '#ffdb99']);
